...
const Banner = () => {
        ...
        const fetchBanners = async () => {
            try {
                ...
                if (response.data.success) {
                    ...
                    const activeBanners = response.data.data.filter(banner => banner.status);
    ...
    if (loading) {
        return (
            <div className="w-full h-[450px] bg-gray-100 animate-pulse flex items-center justify-center">
        ...
        <div className="relative w-full h-[450px] overflow-hidden rounded-lg shadow-md">
            ...
            <div className="relative w-full h-full bg-gray-100">
                {banners.map((banner, index) => (
                    <div
                        key={banner.id}
                        className={`absolute top-0 left-0 w-full h-full transition-all duration-700 ease-in-out transform ${index === currentIndex
                    ...
                    >
                        <img
                            src={banner.image_url || `${import.meta.env.VITE_API_BASE_URL}/${banner.image}`}
                            alt="Banner"
                            className="w-full h-full object-contain"
                            onError={(e) => {
                                console.error('Lỗi load ảnh:', e);
                                console.log('URL ảnh gốc:', banner.image_url || `${import.meta.env.VITE_API_BASE_URL}/${banner.image}`);
                                e.target.onerror = () => {
                                    e.target.src = 'https://via.placeholder.com/800x450?text=Banner+Image+Error';
                                };
                            }}
                        />
                    </div>
