    ...
    const loadChatHistory = async (userId) => {
        ...
        try {
            ...
            if (response.data && Array.isArray(response.data)) {
                ...
                // Nhóm các tin nhắn ảnh từ admin
                const groupedMessages = [];
                let currentGroup = null;

                messagesWithReadStatus.forEach((msg) => {
                    // Nếu là tin nhắn ảnh từ admin
                    if (analyzeAdminMessages(msg) && msg.image && !msg.text) {
                        // Kiểm tra xem có thể thêm vào nhóm hiện tại không
                        if (currentGroup && 
                            Math.abs(new Date(msg.sent_at) - new Date(currentGroup.messages[0].sent_at)) < 120000) {
                            currentGroup.messages.push(msg);
                        } else {
                            // Nếu có nhóm cũ, thêm vào kết quả
                            if (currentGroup) {
                                // Tạo tin nhắn nhóm ảnh
                                if (currentGroup.messages.length > 1) {
                                    const groupId = `group-admin-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
                                    groupedMessages.push({
                                        id: groupId,
                                        sender_id: currentGroup.messages[0].sender_id,
                                        isAdmin: true,
                                        sent_at: currentGroup.messages[0].sent_at,
                                        is_read: currentGroup.messages[0].is_read,
                                        imageGroup: {
                                            urls: currentGroup.messages.map(m => m.image),
                                            groupId: groupId
                                        }
                                    });
                                } else {
                                    // Nếu chỉ có 1 ảnh, giữ nguyên tin nhắn
                                    groupedMessages.push(currentGroup.messages[0]);
                                }
                            }
                            // Bắt đầu nhóm mới
                            currentGroup = {
                        ...
                        // Nếu có nhóm đang xử lý, thêm vào kết quả
                        if (currentGroup) {
                            if (currentGroup.messages.length > 1) {
                                const groupId = `group-admin-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
                                groupedMessages.push({
                        ...
                        // Thêm tin nhắn không phải ảnh vào kết quả
                ...
                // Cập nhật messages với tin nhắn đã được nhóm
